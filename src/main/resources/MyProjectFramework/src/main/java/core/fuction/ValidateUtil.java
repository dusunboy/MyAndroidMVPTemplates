...
public class ValidateUtil {
	...
	/**
	 * TextView,EditView文本内容是否为空
	 * @param tv
	 * @param displayStr
	 * @return
	 */
	public static boolean isEmpty(TextView tv, String displayStr) {
		if (StringUtil.empty(tv.getText().toString().trim())) {
			tv.setError(displayStr);
	 ...
	 * @return
	 */
	public static boolean isMobileNumber(TextView w, String displayStr) {
		if (!RegexUtil.isMobileNumber(w.getText().toString().trim())) {
			w.setError(displayStr);
			w.setFocusable(true);
			return false;
		}
		return true;
